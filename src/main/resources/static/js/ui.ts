...
export function toggleSubscription(e: Event) {
    ...
    if (deviceId !== Store.currentSubscriptionId || deviceId === Store.currentSubscriptionId) {
        ...
        if (deviceId === Store.currentSubscriptionId) return;
    }
...
export function openOrUpdatePopup(text: string) {
    if (!Store.isPopupSwitchTurnedOff) return;
...
export function closePopup(element?: Element) {
    const popup = document.querySelector('.popup-wrapper') ??
        throwNullReferenceError('Popup wrapper is not found');
    const span = popup.querySelector('span#popup-text') ??
        throwNullReferenceError('Popup span is not found');
    ...
    if (element === document.querySelector('.popup-switch-container')) {
        const popupSwitch = document.querySelector('.popup-switch') ??
            throwNullReferenceError('Popup switch is not found');
